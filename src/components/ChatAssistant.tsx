...
export const ChatAssistant: React.FC<ChatAssistantProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="bg-amber-50/90 border-b border-amber-200 px-3 py-1.5 flex items-center gap-2 text-xs text-amber-900">
      ...
      {/* Messages Container */}
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50 text-base">
        {messages.map((msg) => {
          ...
          return (
            ...
            >
              {!isUser && (
                <div className="w-7 h-7 rounded-full bg-blue-700 text-white flex items-center justify-center shrink-0 text-sm shadow-2xs mt-0.5">
              ...
              >
                {isUser ? (
                  ...
                  <div className="text-base prose prose-sm max-w-none text-slate-800 leading-relaxed break-words">
              ...
              {isUser && (
                <div className="w-7 h-7 rounded-full bg-slate-700 text-white flex items-center justify-center shrink-0 text-sm shadow-2xs mt-0.5">
        ...
        {isLoading && (
          <div className="flex items-center gap-2.5 text-sm text-slate-500 italic bg-white p-3 rounded-2xl border border-slate-200 w-fit">
      ...
      {messages.length <= 1 && (
        <div className="bg-white border-t border-slate-200 px-3 py-2 overflow-hidden">
          <div className="flex overflow-x-auto items-center gap-2 pb-1 hide-scrollbar">
            <span className="text-slate-400 font-semibold uppercase text-xs shrink-0 flex items-center">
            ...
            {QUICK_PROMPTS.slice(0, 4).map((qp, idx) => (
              <button
                ...
                disabled={isLoading}
                className="px-3 py-1.5 rounded-full bg-slate-100 hover:bg-blue-50 text-slate-700 hover:text-blue-700 border border-slate-200 text-xs whitespace-nowrap transition-colors disabled:opacity-50 shrink-0"
              >
      ...
      >
        <input
          ...
          disabled={isLoading}
          className="flex-1 px-3.5 py-2 text-base bg-slate-50 border border-slate-300 rounded-xl focus:bg-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all disabled:opacity-50 text-slate-900 placeholder:text-slate-400"
        />
        <button
          type="submit"
